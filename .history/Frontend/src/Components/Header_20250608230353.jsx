...
const Header = ({ navbarCategories, navbarLogoParts }) => {
        ...
        <div className="container mx-auto flex items-center justify-between px-6 py-4">
          ...
          <Link to="/" className="flex items-center space-x-2 group">
            ...
            <div className="text-2xl md:text-3xl font-black text-gray-100 group-hover:text-red-500 transition-colors duration-300 tracking-tight">
              {navbarLogoParts.part2 || 'Bihar'} <span className="text-blue-400">{navbarLogoParts.part3 || '24/7'}</span>
            </div>
          </Link>

          {/* Navigation links */}
          <ul className={`md:flex space-x-6 font-medium tracking-wide text-white ${isOpen ? 'flex flex-col absolute top-full left-0 w-full bg-gray-900 p-4 md:static md:flex-row md:p-0' : 'hidden'}`}>
