...
export const HeroBanner = ({ heroBanner }) => {
  ...
  return (
    <section className="relative h-screen w-full flex items-center justify-center text-white overflow-hidden">
      ...
      <div className="absolute bg-black/80 inset-0 z-10 flex flex-col items-center justify-center px-6 text-center">
        <motion.h1 
        ...
        >
          <button 
            ...
            className="bg-white text-blue-800 px-8 py-3 rounded-full font-semibold hover:bg-gray-100 transition-colors"
          >
      ...
      <motion.div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {latestNewsArticles.length > 0 ? latestNewsArticles.map((article, i) => (
          ...
          >
            ...
            <div className="p-6">
              <h3 className="font-bold text-xl mb-3 hover:text-indigo-600 transition-colors line-clamp-2">
                {article.title}
              </h3>
              <div className="text-gray-600 mb-4">
                <p className="line-clamp-3">
                  ...
                  {article.description.split(' ').length > 15 && (
                    <span 
                      className="text-indigo-600 hover:text-indigo-800 font-medium ml-2 cursor-pointer inline-flex items-center"
                      onClick={(e) => { 
                        e.stopPropagation(); 
...
export const PhotoGallery = ({ photoGalleryTitle, galleryPhotos }) => (
  ...
  >
    <div className="flex justify-between items-center max-w-[1250px] mx-auto mb-14">
    ...
    <motion.div className="grid grid-cols-2 md:grid-cols-4 mx-15 gap-4">
    ...
    >
      ...
      <motion.div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {localNewsArticles && localNewsArticles.length > 0 ? localNewsArticles.map((article, i) => (
          ...
          <p>No local news articles available.</p>
        )}
      </motion.div>
    </motion.section>
  );
...
export const VideoNews = ({ videoNewsTitle, videos }) => {
  ...
  return (
    <section id="video-news" className="py-16 px-6 container w-full max-w-[1300px] mx-auto">
      <div className="flex justify-between items-center mb-8">
        <h2 className="text-4xl font-bold uppercase block text-gray-900 tracking-tight gap-4">
          {videoNewsTitle}
          <span className="block w-24 h-1 mt-4 bg-gradient-to-r from-pink-500 to-yellow-400 rounded-full"></span>
        </h2>
        <a href="#" className="text-indigo-600 hover:underline font-semibold text-base flex items-center gap-2">
          View All
        </a>
      </div>
      <div className="relative">
        <div ref={sliderRef} className="keen-slider flex gap-2">
          {videos && videos.length > 0 ? videos.map((url, i) => (
            <div key={i} className="keen-slider__slide flex-shrink-0 w-full md:w-auto">
