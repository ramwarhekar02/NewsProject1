    ...
    <section className="relative h-screen flex items-center justify-center text-white overflow-hidden">
      <div ref={sliderRef} className="keen-slider absolute inset-0 z-0">
        {heroBanner.images && heroBanner.images.length > 0 ? heroBanner.images.map((image, index) => (
          <div key={index} className={`keen-slider__slide bg-cover bg-center transition-opacity duration-1000`} style={{ backgroundImage: `url(${image})`, opacity: index === currentSlide ? 1 : 0, filter: 'brightness(0.7)' }}></div>
        )) : (
          <div className="keen-slider__slide bg-cover bg-center" style={{ backgroundImage: `url('https://source.unsplash.com/random/1920x1080?news')`, filter: 'brightness(0.7)' }}></div>
        )}
      </div>
