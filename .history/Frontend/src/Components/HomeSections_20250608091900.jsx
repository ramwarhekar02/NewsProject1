    ...
    <section className="relative h-screen flex items-center justify-center bg-gradient-to-r from-blue-800 to-purple-700 text-white">
      <div ref={sliderRef} className="keen-slider w-full h-full absolute inset-0">
        {heroBanner.images && heroBanner.images.length > 0 ? heroBanner.images.map((image, index) => (
          <div key={index} className="keen-slider__slide w-full h-full bg-cover bg-center" style={{ backgroundImage: `url(${image})` }}></div>
        )) : (
          <div className="keen-slider__slide w-full h-full bg-cover bg-center" style={{ backgroundImage: `url('https://source.unsplash.com/random/1920x1080?news')` }}></div>
        )}
      </div>
