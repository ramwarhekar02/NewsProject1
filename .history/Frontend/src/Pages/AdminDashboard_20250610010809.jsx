...
const AdminDashboard = () => {
  ...
  // Keep only marqueeItems for admin to edit live lines one by one
  ...
  const fetchCustomization = async (authToken) => {
    try {
      const response = await fetch('http://localhost:5000/api/customization', {
      ...
      if (response.ok) {
        ...
        // setNavbarCategories(data.navbarCategories || ['Home', 'About', 'Contact']);
        // setNavbarLogoParts(data.navbarLogoParts || { part1: 'News', part2: 'Bihar', part3: '24/7' });
        // setFooterLogoParts(data.footerLogoParts || { part1: 'News', part2: 'Bihar', part3: '24/7' });
        // setFooterDescription(data.footerDescription || 'Delivering the latest news with clarity and speed. Stay informed with breaking news, in-depth analysis, and exclusive stories.');
        // setFooterContactInfo(data.footerContactInfo || { email: 'contact@newsbihar247.com', phone: '+91 12345 67890', address: '123 News Street, Bihar, India' });
        // setFooterQuickLinksCategories(data.footerQuickLinksCategories || ['Home', 'About', 'Contact', 'Advertise']);
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4 md:p-8 overflow-auto space-y-6 md:space-y-8">
      ...
      <section className="bg-white p-4 md:p-6 rounded shadow-md">
        <h2 className="text-xl md:text-2xl font-semibold mb-3 md:mb-4">Navigation Menu</h2>
        <button
          onClick={() => setIsNavOpen(!isNavOpen)}
          className="mb-4 bg-blue-600 text-white px-3 py-2 rounded hover:bg-blue-700 transition text-sm md:text-base"
        >
        ...
        {isNavOpen && (
          <>
            <div className="mb-4">
              <label className="block font-semibold mb-1 text-sm md:text-base">Navbar Logo Part 1</label>
              <input
                type="text"
                value={navbarLogoParts.part1}
                onChange={(e) => setNavbarLogoParts({ ...navbarLogoParts, part1: e.target.value })}
                className="w-full px-2 py-1 md:px-3 md:py-2 border rounded mb-2 text-sm md:text-base"
              />
              <label className="block font-semibold mb-1 text-sm md:text-base">Navbar Logo Part 2</label>
              <input
                type="text"
                ...
                className="w-full px-2 py-1 md:px-3 md:py-2 border rounded mb-2 text-sm md:text-base"
              />
              <label className="block font-semibold mb-1 text-sm md:text-base">Navbar Logo Part 3</label>
              <input
                type="text"
            ...
            <div className="mb-4">
              <label className="block font-semibold mb-1 text-sm md:text-base">Navbar Categories (comma separated)</label>
              <input
                type="text"
            ...
            <ul className="space-y-2">
              {footerLinks.map((link, index) => (
                <li key={index} className="flex flex-col md:flex-row md:items-center md:space-x-2">
                  <input
                    type="text"
                    value={link.name}
                    onChange={(e) => {
                    ...
                    className="flex-1 px-2 py-1 md:px-3 md:py-2 border rounded mb-2 md:mb-0 text-sm md:text-base"
                  />
                  <input
                    type="text"
                    value={link.url}
              ...
              <button
                ...
                type="button"
                className="mt-2 bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700 transition text-sm md:text-base"
              >
                Add Link
              </button>
            </ul>
      ...
      <section className="bg-white p-6 rounded shadow-md">
        ...
        {/* Removed footer editing from admin side as per request */}
      </section>
